...
<div class="page-header">
    <h1 class="page-title">Invoiced Orders(@php
        echo count($Orders);
        @endphp)</h1>
</div>
<!-- PAGE-HEADER END -->
        ...
        <div class="col-md-12 mt-3">
            <div class="card-toolbar">
                <form action="{{ route('assigned.status') }}" method="post" id="myForm">
                    ...
                    <input type="hidden" name="status" value="PACKED" class="form-control">
                ...
                <form action="{{ route('jandt.assign') }}" method="post" style="float: right">
                    @csrf
                    <input type="hidden" name="ids" class="ids" id="ids" value="">
                    <button class="btn btn-sm btn-secondary m-1" href=""><i class="fa fa-user"></i>
                        J&T Assign</button>
                </form>
            </div>
        </div>
                ...
                <tbody class="fw-6 fw-semibold text-gray-600">
                    ...
                    <tr class="{{ getClass($order->status) }}">
                        <th scope="row" class="tdClass"><input type="checkbox" class="sub_chk"
                            ...
                            #{{ $order->order_id }} &nbsp; {!! $generator->getBarcode($order->order_id, $generator::TYPE_CODE_128) !!}</th>
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success == true) {
                    ...
                    if ('INVOICE PRINTED' == response.details['status']) {
                        document.getElementById("myForm").submit();
                    }
                }
            }
