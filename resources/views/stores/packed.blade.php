...
@section('content')
    ...
    <div class="page-header">
        <h1 class="page-title">orders(@php
            echo count($Orders);
        @endphp)</h1>
    </div>
    <!-- PAGE-HEADER END -->
    <div class="card ml-2 mt-5 mb-5 mb-lg-10">
        <div class="card-header row">
           ...
           <div class="card-toolbar">
                <form action="{{ route('stores.assigned') }}" method="post" id="myForm">
                    @csrf
                    ...
                    <input type="hidden" name="status" value="ASSIGNED TO RIDER" class="form-control">
                    <label for="date" class="mt-2">Delivery Date</label>
                    <input type="date" name="delivery_date" id="delivery_date" class="form-control"
                        value="{{ $delivery_date }}" required>
                    <label for="driver" class="mt-2">Select Driver</label>
                    <div class="col-12">
                        <select name="delivery_boy_id" id="delivery_boy_id" class="form-control" required>
                            <option value="">select driver</option>
                            @foreach ($drivers as $item)
                                <option value={{ $item->id }} @if ($selected_rider == $item->id) selected @endif>
                                    {{ $item->name }}</option>
                            @endforeach
                        </select>
                    </div>
